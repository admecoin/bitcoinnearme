...
class WalletController extends Controller {
	...
	public function index() {
    	...
    	if ($uid != '') {
			...
			foreach ($coins as $c) {
				...
				$withdrow_balance = DB::table('a_withdraw_list')->select('amount')->where('user_id', $uid)->where('coin_id', $c->id)->sum('amount');
				if ($withdrow_balance == NULL) {
					$withdrow_balance = 0;
				}
		    	...
		    	if (count($wallet) > 0) {
		    		foreach ($wallet as $w) { }
			    		...
			    		$array[] = array(
			    			...
			    			"buy" => $buy,
			    			"fees" => $fees,
							"locked" => $locked+$withdrow_balance,
	...
	public function lock_balance($coin_id) {
		...
		if(count($wallet)>0) {
			...
			$ethaddr = DB::table('a_coin_wallet')->where('user_id', $uid)->where('coin_id', "2")->get();   
			$withdrow_balance = DB::table('a_withdraw_list')->where('user_id', $uid)->where('coin_id', "2")->where('status', "1")->sum('amount');
